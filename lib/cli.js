...
const DEFAULT_LOCATION = 'Dulles:Chrome';
const DEFAULT_CONNECTIVITY = 'Cable';
const DEFAULT_WEBPAGETEST_HOST = 'https://www.webpagetest.org';
const DEFAULT_NAMESPACE = 'webpagetest';
const DEFAULT_MEDIANRUN = 'fastest';
const DEFAULT_MEDIAN = 'SpeedIndex';
...
const DEFAULT_METRICS = 'SpeedIndex,render,TTFB,domComplete,fullyLoaded,lastVisualChange,visualComplete95,visualComplete99,FirstInteractive,TimeToInteractive,LastInteractive';
...
module.exports = {
	...
	help: function() {
		...
		console.log( '   --median             Choose the metric used to calculate median for' +
	...
	validateArgs: function( argv ) {
		...
		} else if ( argv._length > 1 ) { // eslint-disable-line no-underscore-dangle
			console.error( 'There are ' + argv._length + ' arguments passed to the script (one is' + // eslint-disable-line no-underscore-dangle
		...
		if ( !argv.batch ) {
			...
			const isOK = reporter.validate( argv );
			if ( !reporter ) {
