...
module.exports = {
    ...
    help: function() {
        ...
        console.log(' You can test multiple URL:s (or files) supplying them like: [options]' +
        ' URL1 URL2 URL3\n');
        console.log(' Options:');
        console.log('   --webPageTestKey     The secret key for the WebPageTest instance ' +
         '[required]');
        console.log('   --namespace          The namespace of the key sent to statsv.' +
        '[webpagetest]');
        console.log('   --location           The location and browser to use, check ' +
        ' http://wpt.wmftest.org/getLocations.php [us-west-2:Chrome]');
        ...
        console.log('   --userStatus         Is the user logged in or not? Used in the namespace ' +
        ...
        console.log('   --dryRun             Send metrics to statsv or not. Set to anything if ' +
    ...
    isArgsOK: function(argv) {
        ...
        if (argv.userStatus && AVAILIBLE_USER_STATUS.indexOf(argv.userStatus) === -1) {
            console.error('Not a valid user status:' + argv.userStatus + ' Needs to be one of ' +
