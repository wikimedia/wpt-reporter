...
module.exports = {
  ...
  // https://sites.google.com/a/webpagetest.org/docs/advanced-features/webpagetest-restful-apis#TOC-Sample
  ...
  METRICS : ['SpeedIndex', 'render', 'firstPaint', 'TTFB', 'fullyLoaded', 'loadTime', 'visualComplete'],
  ...
  setupWPTOptions: function(argv) {
    ...
    Object.keys(argv).forEach(function(param) {
      if (['webPageTestKey', 'webPageTestHost', '_', 'verbose', 'userStatus', 'dryRun', 'customMetrics', 'namespace'].indexOf(param) === -1) {
  ...
  collectMetrics: function(wptJson, userStatus, namespace) {
    ...
    VIEWS.forEach(function(view) {
      // if we are missing browser info from WPT (happens when using MotoG at least) use a normalized version of the location
      // the browser/location can then look like Dulles_MotoG_Motorola_G___Chrome but since there are no standard of naming
      // it should be ok to just use what we got
      var browser = BROWSERS[wptJson.data.median[view].browser_name] || wptJson.data.location.replace(/[^A-Za-z_]/g, '_');

      self.METRICS.forEach(function(metric) {
        metricsToSend.push(namespace + '.' + userStatus + '.' + browser + '.' + view + '.' + metric + '=' + wptJson.data.median[view][metric] + 'ms');
      });
      ...
      if (wptJson.data.median.firstView.userTimes) {
        Object.keys(wptJson.data.median.firstView.userTimes).forEach(function(userTiming) {
          metricsToSend.push(namespace + '.' + userStatus + '.' + browser + '.' + view + '.' + userTiming + '=' + wptJson.data.median[view].userTimes[userTiming] + 'ms');
        });
      }
    });
